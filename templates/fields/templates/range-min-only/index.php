<div class="flex-box">
    ...
    <?if($table == 'c_industry_parts' ){?>
        ...
        <div class="flex-box" style="border: 1px solid #E0E0DD; background: #ffffff;">
            <div>
                <input step="0.1" <?if($min !==''){?> min="<?=$min?>" <?}?>  <?if($max !==''){?>max="<?=$max?>"  <?}?>  <?//=$field_max_value?> class="filter-input input-range"  <?=($field->getField('field_required') ? 'required' : '')?> name="<?=$field->title()?>_min" value="<?=$src[$field->title().'_min'] ?? $value_min?>"  type="number" placeholder=" " />
            </div>
        </div>
        <div style="display: none;">
            -
        </div>
        <div class="flex-box" style="border: 1px solid #E0E0DD;  background: #ffffff; display: none;">
            <div>
                <input step="0.1" <?if($min !==''){?> min="<?=$min?>" <?}?>  <?if($max !==''){?>max="<?=$max?>"  <?}?>  <?//=$field_max_value?> class="filter-input input-range" <?=($field->getField('field_required') ? 'required' : '')?> name="<?=$field->title()?>_max" value="<?=$src[$field->title().'_max'] ?? $value_max?>"   type="number" placeholder=" " />
            </div>
        </div>
    <?}else{?>
        <div class="flex-box" style="border: 1px solid #E0E0DD; background: #ffffff;">
            <div>
                <input step="0.1" min="0" <?//=$field_max_value?> class="filter-input input-range"  <?=($field->getField('field_required') ? 'required' : '')?> name="<?=$field->title()?>_min" value="<?=$src[$field->title().'_min'] ? $src[$field->title().'_min'] : ''?>"  type="number" placeholder=" " />
            </div>
        </div>
        <div style="display: none;">
            -
        ...
        <div class="flex-box" style="border: 1px solid #E0E0DD;  background: #ffffff; display: none">
            <div>
                <input step="0.1" min="0" <?//=$field_max_value?> class="filter-input input-range" <?=($field->getField('field_required') ? 'required' : '')?> name="<?=$field->title()?>_max" value="<?=$src[$field->title().'_max'] ? $src[$field->title().'_max'] : ''?>"  type="number" placeholder=" " />
            </div>
        </div>
    <?}?>
