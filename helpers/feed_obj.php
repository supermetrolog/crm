...
$sql = $pdo->prepare("SELECT * FROM c_industry WHERE region IN(1,2) AND deleted!='1' AND result IN(0,5)  ORDER BY dt_update_full DESC  LIMIT 100");
...
////ПЕРЕБИРАЕМ ПОДХОДЯЩИЕ БЛОКИ ВНУТРИ ОБЪЕКТА
    ...
    $blocks_sql = $pdo->prepare("SELECT * FROM c_industry_blocks WHERE parent_id='".$object_item->id."' AND import_cian='1' AND deleted!='1' AND result IN(0,5) ");
    $blocks_sql->execute();
...
$dom->save('xml/cian_feed_210.xml'); // сохранение файла

echo $blocks_count.'<br>';
echo $objects_count.'<br>';
