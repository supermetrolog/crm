...
//$dsn = "mysql:unix_socket=/var/run/mysqld/mysqld.sock;dbname=$db;charset=$charset";
    ...
    //ХУйня для цен офисов и мезанина
    /*
    $sql = $pdo_old->prepare("SELECT * FROM c_industry_blocks WHERE deleted!='1' ORDER BY id ASC ");
    $sql->execute();
    while($block = $sql->fetch(PDO::FETCH_LAZY)){
        $id=$block->id;

        $sql_old = $pdo_old->prepare("SELECT * FROM c_industry_blocks WHERE id=$id ");
        $sql_old->execute();
        $block_old = $sql_old->fetch(PDO::FETCH_LAZY);

        $block_obj = new Subitem($id);

        $price_mezz_min = $block_old->mezz_price;
        $price_mezz_max = $block_old->mezz_price;

        $block_obj->updateField('price_mezzanine_min',$price_mezz_min);
        $block_obj->updateField('price_mezzanine_max',$price_mezz_max);

        $price_office_min = $block_old->office_price;
        $price_office_max = $block_old->office_price;

        $block_obj->updateField('price_office_min',$price_office_min);
        $block_obj->updateField('price_office_max',$price_office_max);


    }


    //Электричесво по блокам
    ...
    while($object = $sql->fetch(PDO::FETCH_LAZY)) {
        ...
        $sql_block = $pdo->prepare("SELECT * FROM c_industry_blocks WHERE object_id=$id ");
        $sql_block->execute();
        while($block = $sql_block->fetch(PDO::FETCH_LAZY)){
            $block_obj = new Subitem($block->id);
            $block_obj->updateField('power',$power);
        }
...
//ШАГ 1
        ...
        $fields_arr = [
            ...
            'area_min' => 'area_floor_min',
            'area_max' => 'area_floor_max',
            'power' => 'power',
            'deal_type' => 'deal_type',
        ...
        while ($item_old = $sql->fetch(PDO::FETCH_LAZY)) {
            ...
            foreach ($fields_arr as $key => $value) {
                ...
                if (in_array($key, ['object_type2'])) {
                    ...
                    foreach($purp_arr_main as $item){
                        if(in_array($item,[1,3,8,10,11,12,13])){
                            $item_type = 1;
                            if(!in_array($item_type,$object_types)){
                                $object_types[] = $item_type;
                            }
                        }
                        if(in_array($item,[2,4,5,6])){
                            $item_type = 2;
                            if(!in_array($item_type,$object_types)){
                                $object_types[] = $item_type;
                            }
                    ...
                    foreach($purp_arr as $item){
                        if($item){
                            ...
                            //смотрим а может здесь подцепим тип объекта
                            $item_type = 1;
                            if(!in_array($item_type,$object_types)){
                                $object_types[] = $item_type;
                            }
...
//ШАГ 4
        ...
        while ($item_old = $sql->fetch(PDO::FETCH_LAZY)) {
            ...
            array_push($keys_arr, 'ad_realtor');
        ...
        while ($item = $sql->fetch(PDO::FETCH_LAZY)) {
            ...
            //смотри все сделки по блокам а нахуя?
            $offers_by_blocks = $obj->getObjectBlocksValuesUnique('deal_type');


            //смотрим типы сделки лота чтобы ориентировать статусы
            ...
            //ДЛЯ ВСЕХ ТИПОВ СДЕЛОК ПО БЛОКАМ(ЧТО ВООБЩЕ СУЩЕСТВУЮТ)
            foreach ($offers_by_blocks as $offer_deal) {
                ...
                //ЕСЛИ И ТИПА СДЕЛКИ БЛОКОВ НЕТУ В ТИПЕ СДЕЛКА ЛОТА ТО СТАТУС ПАССИВ 2
                ...
                $power = (int)$item->power_available;
                ...
                $upd_sql = $pdo->prepare("UPDATE c_industry_blocks SET offer_id='$offer_id', power=$power WHERE object_id='$item->id' AND deal_type='$offer_deal'  ");
                $upd_sql->execute();
                ...
                if($status == 2){
                    ...
                    $upd_sql = $pdo->prepare("UPDATE c_industry_blocks SET status=2 WHERE object_id=$item->id AND deal_type=$offer_deal  ");
                    $upd_sql->execute();
                }
