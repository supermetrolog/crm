...
$ar=readExelFile(PROJECT_ROOT.'/table/industry_rules_14.xls');
...
$towns_central_table='l_towns_central';
$towns_table='l_towns';
...
$sql = $pdo->prepare("TRUNCATE TABLE $towns_central_table");
$sql->execute();
...
//$artificial_regions = ['Москва и МО','Москва внутри МКАД','МО + Москва снаружи мкад','МO + регионы рядом'];
/*
...
//НАСЕЛЕННЫЕ ПУНКТЫ ОСНОВА
$i = 0;
foreach($ar as $ar_row) {
    if ($i > 0 && $i < 100000 && $ar_row[0] != NULL) {
        //НАСЕЛЕННЫЕ ПУНКТЫ главн
        $arr_el = mb_strtolower(trim($ar_row[8]));
        echo $arr_el.'<br>';
        if ($arr_el != NULL && !in_array($arr_el, $towns_central_arr)  ) {
            array_push($towns_central_arr, $arr_el);
            $item = new Post(0);
            $item->getTable('l_towns_central');
            $item->createLine(['title'], [$arr_el]);
...
function getTownByTitle($title, $pdo){
    $town = new Post(0);
    $town->getTable('l_towns');
    $town->getPostByTitle(mb_strtolower(trim($title)));
    return $town->postId();
}
...
foreach($arr_for_eng as $table){
    $post= new Post();
    $post->getTable($table);
    $table_items = $post->getAllUnitsId();
    foreach ($table_items as $item){
        $elem = new Post($item);
        $elem->getTable($table);
        $elem->updateField('title_eng',furl_create($elem->getField('title')));
    }
}
