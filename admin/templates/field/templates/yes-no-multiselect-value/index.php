 <div class="flex-box field-module">
                ...
                <div class="custom-select-body" >
                    <ul class="custom-select-list">
                        ...
                        while($sql_src = $sql->fetch(PDO::FETCH_LAZY)){?>
                            <li>
                                <div>
                                    <input  <?if($src[$field->title()] == 2){?> disabled <?}?>  id="<?=$field->title().'_type'.'-'.$sql_src['id']?>" <?= (in_array($sql_src['id'], json_decode($src[$field->title().'_type'])))? 'checked="checked"' : ''?> value="<?=$sql_src['id']?>" name="<?=trim($field->title().'_type')?>[]" type="checkbox" />
    ...
    <div class=" box-wide field-step-2 <?if($src[$field->title()] != 1){?> hidden <?}?>">
        <div>
            <input   <?if($src[$field->title()] == 2){?> disabled <?}?> type="number" name="<?=trim($field->title())?>_value" value="<?=$src[$field->title().'_value']?>" />
        </div>
    </div>
</div>