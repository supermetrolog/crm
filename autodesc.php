...
foreach($offres_arr as $key=>$value){
    ...
    foreach($blocks_active as $block){
        ...
        */
        ...
        ($offer_block->gf('cross_docking') ==1 ) ? 'Кросс-докинг'  : '' ;
    ...
    //раздел ДОСТУПНОСТИ
    $entrance_line ='<br>';
    ($offer_desc->gf('guard') ==1 ) ? $objGuard='Обьект под охраной ('.$offer_desc->gf('guard').'). ' : $objGuard='' ;
    ($offer_desc->gf('entry_territory') ==1) ? $objEntry=$offer_desc->gf('entry_territory').' въезд на территорию. ' : $objEntry='' ;
