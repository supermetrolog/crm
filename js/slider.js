    ...
    $(document).ready(function(){
    	...
    	for(let i = 0; i < sliders.length; i ++){
            ...
            //для автоустановки начального слайда
            let startPoint = 1;
            startPoint = $(slider).attr('data-slide-start');
            $(slider).find('.slides').animate({scrollLeft : (Math.ceil(startPoint/slides_amount)-1)*$(slider).closest('div').width()}, slideSpeed);



            //добавляем элементы управления горизонтальные
			if($(slider).hasClass('horizontal-arrows')){
                $(slider).prepend('<div class="slider-nav slider-prev background-fix" data-scroll="-1"></div>');
                $(slider).prepend('<div class="slider-nav slider-next background-fix" data-scroll="1"></div>');
			}


            //добавляем элементы управления вертикальные
            if($(slider).hasClass('vertical-arrows')){
                $(slider).prepend('<div class="slider-nav-vertical slider-up background-fix" data-scroll="-1"></div>');
                $(slider).prepend('<div class="slider-nav-vertical slider-down background-fix" data-scroll="1"></div>');
            }
