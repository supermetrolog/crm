<div class="flex-box field-module">
    ...
    <div class="field-step-2 <?if($src[$field->title()] != 1){?> hidden <?}?>">
        <select style="max-width: 120px;" id="field-<?=$field->title()?>" class="<?= (trim($value_item)) ? 'field-checked' :'' ?>" <?=($field->getField('field_required') ? 'required' : '')?> name='<?=$field->title()?>_type' <?=($field->getField('field_is_disabled')) ? 'disabled' : '' ?>>
            <?
    ...
    <div class="box-wide field-step-2 <?if($src[$field->title()] != 1){?> hidden <?}?>">
        <div>
            <input type="number" name="<?=trim($field->title())?>_value" value="<?=$src[$field->title().'_value']?>" />
        </div>
    </div>
</div>