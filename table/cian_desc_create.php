...
foreach($offres_arr as $key=>$value){
    ...
    ($offer_desc->gf('metro')) ? $objMetro='Ближайшее метро '.mb_convert_case($offer_desc->gf('metro'),MB_CASE_TITLE, "UTF-8") : $objMetro='' ;
    ...
    ($offer_desc->gf('area_max')) ? $objAreaAll= valuesCompare($offer_desc->gf('area_min'),$offer_desc->gf('area_max')).' м. кв.' : $objAreaAll='' ;
    ...
    foreach($blocks_active as $block){
        ...
        */
        ...
        ($offer_block->gf('load_mezzanine_min')) ? $objLoadMezz=', на мезонин '.valuesCompare($offer_block->gf('load_mezzanine_min'),$offer_block->gf('load_mezzanine_max')).' т/м.кв.'  : $objLoadMezz='' ;
        ($offer_block->gf('cross_docking')) ? $crossDock='Кросс-докинг'  : $crossDock='' ;
        ($offer_block->gf('column_grid')) ? $columnGrid='Сетка колонн '.$offer_block->gf('column_grid')  : $columnGrid='' ;
        ($offer_block->gf('warehouse_equipment')) ? $equipment='Возможна аренда складской техники.'  : $equipment='' ;
        ($offer_block->gf('charging_room')) ? $chargeRoom='Имеется зарядная комната.'  : $chargeRoom='' ;
        $main_line.= $objArea.
            $objMezz .$mezzAdd.
            $objMeasure.
            $objLandscape.
    ...
    ($offer_desc->gf('gas')) ? $objGas='ГАЗ, ' : $objGas='' ;
    ($offer_desc->gf('steam')) ? $objSteam='ПАР, ' : $objSteam='' ;
    ($offer_desc->gf('water')) ? $objWater='вода, ' : $objWater='' ;
    ($offer_desc->gf('sewage_central')) ? $objSewage='канализация, ' : $objSewage='' ;
    ($offer_desc->gf('ventilation')) ? $objVent=$offer_desc->gf('ventilation').' вентиляция, ' : $objVent='' ;
    ($offer_desc->gf('fire_alert')) ? $objFireAlert='пожарная сигнализация, ' : $objFireAlert='' ;
    ($offer_desc->gf('smoke_exhaust')) ? $objSmoke='дымоудаление, ' : $objSmoke='' ;
    ($offer_desc->gf('firefighting')) ? $objFireFight=$offer_desc->gf('firefighting').', ' : $objFireFight='' ;
    ($offer_desc->gf('video_control')) ? $objVideoControl='видеонаблюдение, ' : $objVideoControl='' ;
    ($offer_desc->gf('security_alert')) ? $objSecurityAlert='охранная сигнализация, ' : $objSecurityAlert='' ;
    ($offer_desc->gf('access_control')) ? $objAccessControl='контроль доступа на территорию, ' : $objAccessControl='' ;
    ...
    ($offer_desc->gf('hostel')) ? $objHostel='общежитие, ' : $objHostel='' ;
    ($offer_desc->gf('canteen')) ? $objCanteen='столовая, ' : $objCanteen='' ;
    ($offer_desc->gf('railway')) ? $objRailway='действующая Ж/Д ветка, ' : $objRailway='' ;
    ($offer_desc->gf('cranes_railway_num')) ? $objRailCranes='Ж/Д краны, ' : $objRailCranes='' ;
