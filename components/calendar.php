  ...
  for($list_day = 1; $list_day <= $days_in_month; $list_day++):
    $calendar.= '<td class="calendar-day">';
      ...
      /** ЗДЕСЬ МОЖНО СДЕЛАТЬ MySQL ЗАПРОС К БАЗЕ ДАННЫХ! ЕСЛИ НАЙДЕНО СОВПАДЕНИЕ ДАТЫ СОБЫТИЯ С ТЕКУЩЕЙ - ВЫВОДИМ! **/
	   $events_sql = $pdo->prepare("SELECT * FROM core_tasks WHERE start_time<'$date_to_find_before' AND end_time>='$date_to_find' ");
       $events_sql->execute();	   
	   while($event = $events_sql->fetch()){
        ($event['finished']) ? $done ='<i title="Завершено" class="good fas fa-check"></i>' : $done='';
        ($event['mark_as_important']) ? $important ='<i title="Важное" class="attention box-wide fas fa-exclamation"></i>' : $important ='';
		 $calendar.= "<p>$important $done<a href='PROJECT_URL/task/".$event['id']."'><b>".$event['title']."</b></a></p>";
	   }  
