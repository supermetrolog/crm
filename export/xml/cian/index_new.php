...
$sql_text = "SELECT * FROM $table WHERE type_id=2 AND ad_cian=1  AND  deleted!='1'  AND status=1 AND is_land!=1 AND (price_floor_min > 0 OR  price_sale_min > 0)  AND test_only!=1 AND photos!='[\"[]\"]'   ";
...
while($offer = $sql->fetch(PDO::FETCH_LAZY)){
    ...
    $TotalArea->appendChild($dom->createTextNode($offer->area_floor_max));
    ...
    */
    ...
    if ($offer->area_floor_min != $offer->area_floor_max) {
        ...
        $Area->appendChild($dom->createTextNode($offer->area_floor_min));

        $Price = $RentByPartsSchema->appendChild($dom->createElement('Price'));
        $Price->appendChild($dom->createTextNode($offer->price_floor_min_month_all));

        $VatPrice = $RentByPartsSchema->appendChild($dom->createElement('VatPrice'));
        $VatPrice->appendChild($dom->createTextNode(0)); //хз чо писать


        //здесь указать другие разновидности


        $RentByPartsSchema = $AreaParts->appendChild($dom->createElement('RentByPartsSchema'));
        $Area = $RentByPartsSchema->appendChild($dom->createElement('Area'));
        $Area->appendChild($dom->createTextNode($offer->area_floor_max));

        $Price = $RentByPartsSchema->appendChild($dom->createElement('Price'));
        $Price->appendChild($dom->createTextNode($offer->price_floor_max_month_all));
    ...
    */
    ...
    if(in_array(1,$object_types) && in_array(2,$object_types)){
        ...
        $title_first = 'Производство от ';
    }else {
        $title_first = 'Склад от ';
    }
    $min_area = $offer->area_floor_min;
    $title_first = $title_first .$min_area . 'м.кв.';
    ...
    if($offer->deal_type ==1 || $offer->deal_type == 1 ){
        //ПЕРИОД ОПЛАТЫ
