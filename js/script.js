...
function setDatalistVariant() {
	...
	if (elem) {
		...
		if (table === 'c_industry_companies' && elem.closest('.datalist-field').querySelector('input').getAttribute('data-company') == 1) {
			...
			// let result = sendAjaxRequestPost('http://crm/templates/fields/datalist-items/index.php', params, false);
...
function getDatalistVariants() {
	let elem = event.target.closest('.datalist-input-api');
	if (elem) {
		...
		if (elem.getAttribute('data-async') == 1) {
			...
			result = sendAjaxRequestPost('https://pennylane.pro/templates/fields/datalist-api-items/index.php', params, false);
			// result = sendAjaxRequestPost('http://crm/templates/fields/datalist-items/index.php', params, false);
...
//Подгрузить список вариантов в поле даталист
function getDatalistVariantsApi() {
	let elem = event.target.closest('.datalist-input-api');
	if (elem) {
		...
		if (elem.getAttribute('data-async') == 1) {
			...
			result = sendAjaxRequestPost('https://pennylane.pro/templates/fields/datalist-api-items/index.php', params, false);
			// result = sendAjaxRequestPost('http://crm/templates/fields/datalist-api-items/index.php', params, false);
