        ...
        if (table === 'c_industry_companies' && elem.closest('.datalist-field').querySelector('input').getAttribute('data-company') == 1) {
            ...
            res.then((response) => {
                ...
                .then((data) => {
                    ...
                    let array = [];
                    data.forEach((el) => {
                            array.push([el.id, el.full_name == null ? "Общий" : el.full_name]);
                        })
                        //alert(result);
...
function getDatalistVariants() {
    ...
    if (elem) {
        ...
        console.log('получение не из API ');
...
function getDatalistVariantsApi() {
    ...
    if (elem) {
        ...
        if (elem.getAttribute('data-async') == 1) {
            ...
            // result = sendAjaxRequestPost('http://objects/templates/fields/datalist-api-items/index.php', params, false);
